...
> **GitHub state snapshot (2026-06-29) — stale-issue close-out sweep (latest; supersedes older notes where they disagree).** Reviewed 11 maintainer-flagged issues for "already done on `main`?" and closed the ones a merged PR/commit resolved, each with a comment citing the ref:
> - **Closed completed:** **#179** (timestamp-echo scrub — PR #200 / `strip_edge_timestamps`); **#139** (`{user_export}` first-occurrence dedup — PR #200); **#65** (prompt-too-long retry-with-reduced-export — `ac1c9ee`; further removed by the chunked pipeline); **#32** (incremental/chunked profile gen + `source_data_cutoff`/`generation_type`/`parent_profile_id` — PRs #71/#181/#211); **#173** (Part A Batch profile-gen PR #182; Part B agentic prompt caching PRs #198/#199); **#89** (`ProposalInline.js` per-item toggle / inline-add / apply-remainder — shipped under #95/#108/#158; residual: no per-row delete/rename); **#190** (text-mode caching already live — `cache_control` sits on the shared `_is_agentic_prompt` path, not voice-only; the only #187 extra is the finalize pre-warm, which has no sensible text-mode form, so nothing left to build).
> - **#140 — wired to auto-close:** the tiny-first-chunk fallback lives on `feat/145-tts-chapters` (not yet on `main`); open **PR #206** now carries a proper `closes #140` keyword (was "Closes #145 and #140", which only auto-closes #145), so #140 closes when #206 merges.
> - **Kept open — real gaps, documented on the issue:** **#105** — the `stripProposalTag()` stripper is NOT applied to the `### Completed` / `### New Tasks` / `### Priority` bodies or the lead-in/trailing prose (the actual leak path). **This supersedes the 2026-06-10 "#105 dropped, likely no longer reproduces" note — it still reproduces in code.** **#78** — only `beforeunload`; in-app SPA navigation during recording is still unguarded (fix options: `createBrowserRouter`+`useBlocker`, or lighter manual click interception). **#155** — RAG umbrella; only manual Cmd+K search + dark agentic retrieval shipped (PR #197); Community-Archive components 2 & 4 still live; per-chunk embeddings + prod-enable carved into follow-ups #221/#223.
>
...
| **#190** | Text-mode prompt caching | **CLOSED completed (2026-06-29)** | — | Text-mode caching already live on the shared `_is_agentic_prompt` provider path (`cache_control` + OpenAI `prompt_cache_key`); no recording/finalize window → the 3b pre-warm has no sensible text-mode form (drafting-time warm wastes warms on abandoned drafts). Nothing left to build. |
