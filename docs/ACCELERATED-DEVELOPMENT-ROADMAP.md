...
| A.29 | Publicness + provenance at write time (#295 step 0, 2026-09-21) | `ExternalItem.public_source` (True = vouched public by source: Community Archive tweets, X bookmarks with an unprotected author — the sync now requests `protected`; False = web clip; NULL = unknown) and `Node.provenance` on imported nodes (`archive_upload` / `prefill_ca` / `prefill_x`), so the later shared public-reference tier and the open "public tweets public on Loore?" decision can be applied without guessing. Nothing reads them yet; design record and trigger on #295 |
