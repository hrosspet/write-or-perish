...
| A.27 | Account identity hardening (2026-09-15, PRs #304 #305) | Verified email change/add (#260): `POST /api/dashboard/email` sends a link to the NEW address, the address binds only when it is opened (`bind_user_id` claim on the magic-link token, single-use, uniqueness re-checked at click, old address notified); `PUT /dashboard/user` no longer binds an email; X-login accounts can add/remove an email; the alpha waitlist page uses the same flow. Rename-safe permalinks (#253): `username_history` table, former handles 301 to the current one on every public route and in the permalink API, and stay reserved for anyone but their owner |
