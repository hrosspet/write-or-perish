...
| A.26 | Chrome clipper (#232) | MV3 extension: one key press saves the active tab as a reference (`web_clip`, Markdown via Readability+Turndown; tweets → `twitter_bookmark`) and closes it; personal API tokens scoped to `external:write`, minted on the Import page; References log + reference page (tweet embed), user-only Mark as read beside the AI's surfacing history; References search (same modal, `scope=external`, semantic default / keyword admin-only); Mark as read toggle in the inline quote bubble (2026-09-13) |
