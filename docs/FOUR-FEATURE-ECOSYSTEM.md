...
- ✅ **Substrate**: `ExternalItem` (separate from Nodes — references ≠ lore), X OAuth sync (nightly, per-user 3am via `User.timezone`; early-stop pagination ≈ 1 paid request/night; revocation handling; costs metered to `APICostLog`), Community Archive fetch, craft-mode JSON import (deep backfill beyond the API's ~100-item window) with enrichment folding (quoted tweet, link card, media alt, expanded links). **Publicness recorded at write time (2026-09-21, #295 step 0):** `ExternalItem.public_source` — True when the source itself vouches the text is public (Community Archive tweets incl. Read picks; X bookmarks whose author is not protected — the sync now requests `user.fields=protected`), False for a web clip (whatever tab was open, private pages included), NULL = unknown (JSON-imported bookmarks, clipped tweets, rows from before). A clipped tweet whose author wears a protected-account lock arrives as False straight from the extension (`author_protected`, reported only when the lock was seen — a missing lock is never "public"); the remaining unknown tweets are resolved by the nightly `verify_public_source_sweep` against X's free oEmbed endpoint (served → True, refused → False, no answer → retried; runs at 05:00 UTC off an hourly beat gate, ≤300 lookups a night counted per distinct tweet id, each verdict committed as it lands, no API credits, the request carries only the tweet id; the same sweep vouches pre-column Community Archive rows True). The log-side companion is `Node.provenance` on imported nodes (`archive_upload` / `prefill_ca` / `prefill_x`). Both exist so the shared public-reference tier and the open "public tweets public on Loore?" decision (design record on #295) can be applied later without guessing; nothing reads them yet.
...
- ✅ **Digest artifact** (`external_digest`): topic map of the corpus, rebuilt once a night in the user's own night (local ~4am) when the corpus changed — never per saved reference, since each rebuild is one LLM call over the whole corpus — submitted via the provider Batch API (~50% cheaper; a beat collector saves results), read on demand via the artifacts index
- ✅ **Activation**: per-user toggle (`external_content_enabled`, Account page "External references", default off) — the #229 pattern; `SEMANTIC_SEARCH_AGENTIC` env is killswitch-only. Since #329 (2026-09-22) the toggle gates ONLY saved references (whether the search also returns them, the references guidance paragraph, the Chrome clipper); agentic own-archive search (#197) is on for everyone under the killswitch
...
- ✅ **Chrome clipper** (#232, 2026-09-12): MV3 extension in `extension/` — one key press reads the active tab in the browser (Readability → Markdown via Turndown; tweet extractor on x.com), POSTs to `/api/external/clip`, closes the tab. Any page becomes a `web_clip` item (keyed by sha256 of the canonical URL, `title` column added); a tweet URL becomes a `twitter_bookmark` keyed by tweet id so it dedupes against the sync. Auth = personal API token (`ApiToken`, sha256 at rest, scope `external:write` — can add references, cannot read anything; minted/revoked on the Import page). Same 100k-char cap as an entry. References stay out of the profile by construction (separate table). Follow-ups shipped the same day: `/references` Log-shaped list + `/references/:id` page (tweet pages embed the real tweet via X's widget — the one third-party browser call, only on a tweet page the user opened; the log stays text-only), and a user-only **Mark as read** (`read_at`; opening a page is not a read) shown next to the AI's surfacing history (`surfaced_count`/`last_surfaced_at`, bumped when a reply quotes the item). **References search** (2026-09-12): the Log's search button + ⌘K on `/references` open the same modal scoped to saved references only (`scope=external` on `/api/search` and `/api/search/semantic`); semantic by default for everyone, keyword toggle admin-only, as in the Log. Keyword matches titles too; results open the reference page. **Mark as read from the quote** (2026-09-13): the inline quote bubble a reply renders for `{quote_ext:ID}` carries the owner's read toggle in its footer (same endpoint as the reference page), so a recommendation can be marked read where Loore surfaced it instead of only on `/references/:id`. Beside it, **good / bad quote** (`ExternalItem.feedback`: thin ⊕/⊖ contours, muted until chosen; same control on the reference page) — the hit-or-miss half of the recommendation record next to `surfaced_count` (shown) and `read_at` (read). The model sees the verdict as metadata on search previews.

- 🧪 **Community Archive personal feed — PoC** (2026-09-13, local branch `feat/ca-tweets-feed-poc`): the "needle in the haystack" design — read a whole day of the Community Archive (~4.7k tweets, ~250k tokens) against the user's current entry, profile and intentions, and ask whether anything there would change what they do next, with *nothing* as the expected answer. Mechanics: `{ca_tweets?days=N}` prompt placeholder (whole corpus from the nightly parquet snapshot, per-account sections, tweets numbered so picks are cited as `#n` and linked on collect), sent as a one-item Anthropic batch (half price) that the completion task polls. The reply is a verdict plus up to 20 picks; every pick becomes a saved reference, so the existing read mark and good / bad verdict are the feedback channel, and a `FeedPick` row keeps the model's rank, relevance % and recommend flag next to them for calibration (was a 6 % pick good?). Picks render under the verdict as one run of full tweets, relevance in the margin, read ones faded. No nightly job yet. Intention-over-engagement applies with force here: the feed exists to answer "is there anything I'd benefit from reading", not to give the user something to scroll.
  ...
  - **Replies under a Read keep the user's AI usage (#362, 2026-09-25):** the Read's nodes stay 'chat' (they quote other people's tweets), but what the user writes under them no longer inherits that: the default walks past the read prompts and the LLM replies below them to the user's own node (a Read at the root: the account default), in the reply form, Voice, Text mode and the streaming voice path alike. LLM replies in a read thread are stored 'chat', since the tweets are in their context, and cannot be raised to 'train' in the editor or by the "apply to replies" cascade. Existing replies are left as they are.

**Deliberately not built (follow-ups):** overnight batch pre-selection with draft reasoning (#233 — always lags one day, by design), embedding tripwire as spending gate (#231), the extension's *bookmarks-page deep backfill* mode (#232's original ask — the clipper covers one tab at a time; the console exporter + JSON import still covers bulk backfill), plaintext keyword search for public tweets, likes import, og-fetch for cardless links. The 70/30 spaced-repetition blend was replaced by legible age/surfacing metadata + prompt instruction (no hidden fudge factors). MemeOS dependency dropped.
