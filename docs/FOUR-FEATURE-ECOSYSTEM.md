...
- ✅ **Digest artifact** (`external_digest`): topic map of the corpus, rebuilt once a night in the user's own night (local ~4am) when the corpus changed — never per saved reference, since each rebuild is one LLM call over the whole corpus — read on demand via the artifacts index
