...
### Core Features
- **Verified email change + rename-safe permalinks** (#260, #253; 2026-09-15) - Email changes go through a confirmation link to the new address (binds only when confirmed from inside the requesting account's session — the link alone signs nobody in; old address notified; the only unconfirmed write left is the admin's `update_email`); username renames append `username_history` so old `/@handle/...` URLs 301 to the current handle and former handles stay reserved
