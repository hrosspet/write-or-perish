...
Two modes:
  ...
  * --batch — one Batch API request PER USER (~50% cheaper, async, <=24h SLA).
    A batch request can't retry-shrink mid-flight, so we first send ONE sync
    "calibration probe" at the full budget: it overflows (a rejected 400 is
    NOT billed) and returns the REAL token count for free, which we use to
    size the batch export correctly, then submit one right-sized batch. If a
    user's archive happens to fit the probe, we already have the answer and
    just save it (that one runs at full price — it was too small to need a
    batch).
...
def _probe_and_submit(app, user, template, model_id, probe_budget,
                      chronological, api_keys, batch_keys, dry_run, pending):
    """Calibration probe + batch submit for one user.

    Sends one sync request at the full budget. If it overflows (expected for
    a large archive) the rejected 400 returns the REAL token count for free —
    we calibrate the export to fit and submit a right-sized batch (added to
    `pending`). If it fits, we already have the answer and save it (sync, full
    price). Skips with no side effect when the archive is empty or in dry-run.
    """
    ...
    if dry_run:
        print(f"  user {user.id}: probe export ~{est} tokens "
              f"(budget={probe_budget}) — [dry-run] not calling/submitting")
        return
    ...
    try:
        ...
        print(f"  user {user.id}: probe real={e.actual_tokens} > {e.max_tokens} "
              f"max — batch budget={batch_budget}")
        b = _build_messages(user, template, batch_budget, chronological)
        if b is None:
            print(f"  ! user {user.id}: no archive at calibrated budget, skipping")
            return
        b_messages, _, b_est = b
        req = {"custom_id": f"int-u{user.id}", "model_id": model_id,
               "api_model": cfg["api_model"], "messages": b_messages,
        ...
        print(f"  user {user.id}: submitted batch {batch_id} ({provider_key}, "
              f"model={model_id}, export ~{b_est} tokens)")
        return

    # Probe fit in one sync call — already have the result (full price).
    ...
    print(f"  ✓ user {user.id}: fit in one sync call, saved intentions v{version} "
          ...
          f"cost=${cost / 1e6:.4f}, full price — too small to need a batch)")
...
def run_batch(app, users, template, model_override, dry_run,
    ...
    for user in users:
        ...
        if not _eligible(app, user, model_id):
            continue
        try:
            _probe_and_submit(app, user, template, model_id, probe_budget,
                              chronological, api_keys, batch_keys, dry_run,
                              pending)
    ...
    while pending and waited < max_wait:
        ...
        for uid in list(pending):
            ...
            if r:
                ...
                # Batch ended but this request produced no result (calibration
                # should prevent overflow, so this is rare). Report it.
                print(f"  ✗ user {uid}: batch {info['batch_id']} ended with no "
                      f"result — re-run sync for this user")
                pending.pop(uid)
        if pending:
