...
written with the text, and a turn replays it into its licence.
"""
import json
import logging
...
def get_cached_render(config, node, variant=""):
    ...
    try:
        ...
        return CachedRender(entry["text"], entry.get("unlicensed"))
    except Exception:
...
def store_render(config, node, rendered_text, variant="",
    ...
    (see ``CachedRender``). Failures are non-fatal."""
    entry = json.dumps({"text": rendered_text, "unlicensed": unlicensed})
    try:
